...
def train(diffusion_model, train_labels, val_loader, test_loader, device,
    ...
    train_embed = jt.array(np.load(os.path.join(data_dir, 'fp_embed_train_cloth.npy')))
    ...
    optimizer = optim.Adam(params, lr=0.0001, weight_decay=0.0, betas=(0.9, 0.999), eps=1e-08)
    ...
    for epoch in range(n_epochs):
        ...
        # 设置数据集的batch_size和其他参数
        train_dataset.set_attrs(batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, drop_last=True)
        ...
        with tqdm(enumerate(train_dataset), total=len(train_dataset), desc=f'train diffusion epoch {epoch}', ncols=120) as pbar:
            for i, (x_batch, y_batch, _) in pbar:
                ...
                adjust_learning_rate(optimizer, i / len(train_dataset) + epoch, warmup_epochs=1, n_epochs=n_epochs, lr_input=0.001)
                n = x_batch.shape[0]
                ...
                nn.clip_grad_norm_(diffusion_model.diffusion_encoder.parameters(), 1.0)
                optimizer.step()
...
def test(diffusion_model, test_loader, test_embed):
    ...
    if not isinstance(test_embed, jt.Var):
        test_embed = jt.array(test_embed).float32()
...
if __name__ == "__main__":
    ...
    device = args.device if args.device else 'cuda' if jt.has_cuda else 'cpu'
    ...
    train_labels = jt.array(train_dataset.targets).long()
    test_dataset = Clothing1M(data_root=data_dir, split='test')
    val_dataset = Clothing1M(data_root=data_dir, split='val')
    

    print("Warning: Jittor version requires CC_model compatible implementation")
    fp_encoder = CC_model()

    print("Warning: Jittor version requires CC_net model in compatible format")
    # CC_model_dict = jt.load('./model/CC_net.pt')
    # fp_encoder.load_state_dict(CC_model_dict)
    fp_encoder.eval()
    fp_dim = 128


    train_dataset.set_attrs(batch_size=400, shuffle=True, num_workers=args.num_workers, drop_last=True)
    val_dataset.set_attrs(batch_size=400, shuffle=False, num_workers=args.num_workers)
    test_dataset.set_attrs(batch_size=400, shuffle=False, num_workers=args.num_workers)
    ...
    print("Warning: Jittor version requires diffusion model in compatible format")
    # state_dict = jt.load(model_path)
    # diffusion_model.load_diffusion_net(state_dict)

    print('Jittor automatically handles multi-GPU if available')
    ...
    max_accuracy = test(diffusion_model, test_dataset, test_embed)
    print('test accuracy:', max_accuracy)

    # train the diffusion model
    # train(diffusion_model, train_labels, val_dataset, test_dataset, device, model_path, args, data_dir=data_dir)
